...
function bling(){
    worm.style.size ='28px';
    worm.style.color = '#33cc33';
    worm.style.border = '3px odtted rosybrown';
    worm.styl.padding = '10px'; 
}
...
recipe.addEventListener('click', beets);

function beets(){
    var liElm = document.createElement('li');
    liElm.className = 'mix';
    liElm.innerHTML = missing;
    ingredient.appendChild(liElm);
}
...
menu.addEventListener('click', putDessert);
function putDessert(){
...
function addBio(){
    for(var i in grimace){
        var bioSection = document.createElement('div');
        bioSection.id = i;
        bioSection.innerHTML = i + ': ' + grimace[i];
        bio.appendChild(bioSection);
    }
}
...
function randomMessage(){
    ...
    fortune.innerHTML = messages[num];
}
...
secret.addEventListener('keyup', capitalize);
function capitalize(){
